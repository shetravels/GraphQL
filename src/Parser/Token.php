...
class Token
{
     ...
     * @param int $column
     */
    public function __construct(private $type, private $line, private $column, private mixed $data = null)
    {
        if ($this->data) {
            $tokenLength = mb_strlen((string) $this->data);
            $tokenLength = $tokenLength > 1 ? $tokenLength - 1 : 0;

            $this->column = $this->column - $tokenLength;
        }
